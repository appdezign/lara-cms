...
<div class="box box-default">
	...
	<div id="kt_card_collapsible_fieldlist" class="collapse show">
		<div class="box-body pt-4 pb-10">
			...
			<div class="row pb-4">
				<div class="col">
					<a href="{{ route('admin.'.$entity->entity_key.'.reorder', ['id' => $data->object->id]) }}"
					...
					@else
						...
						<div class="row crud-row editcrud-row">
							...
							<div class="col-2 crud-col">
								...
								{{ html()->select('_mhook_'.$field['id'], $data->fieldHooks, old('_mhook_'.$field['id'], $field['fieldhook']))->class('form-select form-select-sm')->data('control', 'select2')->data('hide-search', 'true')->disabled() }}
							</div>
							<div class="col-1 crud-col">
								{{ html()->hidden('_mreq_'.$field['id'], 0) }}
							...
							<div class="col-2 crud-col text-center">
									...
									<div class="builder-field-details mt-3">
										<div class="row">
											...
											<div class="col-2">
												...
												{{ html()->select('_mstate_'.$field['id'], $data->fieldStates, old('_mstate_'.$field['id'], $field['fieldstate']))->class('form-select form-select-sm')->data('control', 'select2')->data('hide-search', 'true')->disabled() }}
											</div>
											<div class="col-2">
												@if($field['fieldstate'] == 'enabledif')
													{{ html()->hidden('_mcondfield_'.$field['id'], $field['condition_field']) }}
													{{ html()->select('_mcondfield_'.$field['id'], $data->fieldList, old('_mcondfield_'.$field['id'], $field['condition_field']))->class('form-select form-select-sm')->data('control', 'select2')->data('hide-search', 'true')->disabled() }}
												@else
													{{ html()->hidden('_mcondfield_'.$field['id'], null) }}
												@endif
											</div>
											...
											<div class="col-2">
												@if($field['fieldstate'] == 'enabledif')
													...
													{{ html()->select('_mcondop_'.$field['id'], ['isequal' => 'is equal to', 'isnotequal' => 'is not equal to'], old('_mcondop_'.$field['id'], $field['condition_operator']))->class('form-select form-select-sm')->data('control', 'select2')->data('hide-search', 'true')->disabled() }}
												@else
													{{ html()->hidden('_mcondop_'.$field['id'], null) }}
												@endif
											</div>
		...
		<div class="box-body py-10">
			...
			@else
				...
				<div class="row crud-row form-group">
					...
					<div class="col-2 crud-col">
						{{ html()->select('_new_fieldtype', $data->fieldTypes, null)->class('form-select form-select-sm')->data('control', 'select2')->data('hide-search', 'true')->disabled() }}
					</div>
					<div class="col-2 crud-col">
						{{ html()->select('_new_fieldhook', $data->fieldHooks, null)->class('form-select form-select-sm')->data('control', 'select2')->data('hide-search', 'true')->disabled() }}
					</div>
					<div class="col-4 crud-col">
